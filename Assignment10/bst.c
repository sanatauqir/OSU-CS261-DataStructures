...
struct Node *_addNode(struct Node *cur, TYPE val)
{
	...
	// add new node if current is null
	if (cur == 0)
	{
		struct Node *newnode;
		newnode = malloc(sizeof(struct Node));
		assert(newnode != 0);  // make sure node exists
		newnode->val = val;
		newnode->left = 0;
		newnode->right = 0;
		return newnode;
	}
		// recursively call addNode to traverse tree
		...
		// return current node for recursive _nodeAddBST() to work
		return cur;
}
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
	...
	while (node != 0)
	{
		...
		else if (compare(val,node->val) == -1)
		{
			// head down left branch
			node = node->left;
		}
		else
		{
			// head down right branch
			node = node->right;
		}
	}
...
TYPE _leftMost(struct Node *cur)
{
	while (cur->left != 0)
	{
		// keep going down the tree following left children until null
		cur = cur->left;
	}
	// leftmost found when cur->left is NULL
	return cur->val;
}
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
	...
	if (compare(val,cur->val) == 0)
	{
		...
		{
			// replace value of current with value of left most child of right child
	...
	{
		// recursive call to keep traversing tree
...
void testRemoveNode() {
	...
	printf("the current count is %d", tree->cnt);
